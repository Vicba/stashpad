...
ENV VIRTUAL_ENV /opt/stash_cli-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/stash_cli/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/stash_cli/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/stash_cli/ && touch src/stash_cli/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/stash_cli/
...
ENTRYPOINT ["/opt/stash_cli-env/bin/poe"]
